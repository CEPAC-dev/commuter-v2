...
export default function CreateClient({ userEmail }: Props) {
  ...
  const [referralDiscountAvailable, setReferralDiscountAvailable] = useState(false);
  const [referralDiscountPercentage, setReferralDiscountPercentage] = useState<number | null>(null);
  const [referralDiscountTripsRemaining, setReferralDiscountTripsRemaining] = useState(0);
    ...
    (async () => {
      ...
      try {
        ...
        if (r.ok) {
          const d = await r.json();
          const info = d?.data;
          setReferralDiscountAvailable(Boolean(info?.referralDiscountAvailable));
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (data?.promoCodeUnavailable) {
        setPromoWarning(data?.promoCodeMessage ?? t("create.promo_unavailable_fallback"));
  ...
  async function handleValidatePromoCode() {
    ...
    try {
      ...
      setPromoCodeMessage(
        result.message ??
          (result.valid
            ? t("create.promo_valid")
            : t("create.promo_invalid")),
      );
  ...
  const appliedReferralSlots =
    useReferralDiscount && referralDiscountAvailable && referralDiscountPercentage != null
        ...
        >
          ...
          >
            ...
            >
              ...
              {referralDiscountAvailable && referralDiscountPercentage != null && (
                <label
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <TicketPercent size={15} color="#00877A" aria-hidden="true" />
                    <span style={{ fontSize: 12.5, color: "#0B1E3D", fontWeight: 700 }}>
                      {t("create.referral_toggle_label")} · {t("create.referral_toggle_offer")
                        .replace("{pct}", String(referralDiscountPercentage))
                        .replace("{count}", String(referralDiscountTripsRemaining))}
                    </span>
                  </span>
                  <input
                    type="checkbox"
                    checked={useReferralDiscount}
                    onChange={(event) => setUseReferralDiscount(event.target.checked)}
              ...
              {totalSavingsEgp > 0 && (
                ...
                >
                  {t("create.discount_savings_label").replace("{amount}", String(totalSavingsEgp))}
                </p>
              )}
              <button
                type="button"
            ...
            <div style={{ padding: "8px 24px 0" }}>
              ...
              {referralDiscountAvailable && referralDiscountPercentage != null && (
                ...
                >
                  ...
                  >
                    <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <TicketPercent size={16} color="#00877A" aria-hidden="true" />
                      <span style={{ fontSize: 13, color: "#0B1E3D", fontWeight: 700 }}>
              ...
              {totalSavingsEgp > 0 && (
                <p style={{ margin: "6px 0 0", fontSize: 12, color: "#00877A", fontWeight: 700 }}>
                  {t("create.discount_savings_label").replace("{amount}", String(totalSavingsEgp))}
                </p>
              )}
