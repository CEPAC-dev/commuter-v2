...
export default function AdminAvailabilityTable({ initialRecords }: { initialRecords: AvailabilityRecord[] }) {
  ...
  return (
    <AdminCard
      ...
      actions={<button type="button" className="avail-btn primary" onClick={() => setCreateOpen(true)}><Plus size={15} /> Add new availability</button>}
    >
      <style>{`
      ...
      {error ? <AdminErrorState title="Unable to update availability" description={error} /> : null}
      {records.length === 0 ? <AdminEmptyState title="No availability records" description="Driver availability submissions will appear here." /> : (
        <AdminTable ariaLabel="Driver availability records">
          <thead style={{ background: "var(--color-surface)" }}>
            <tr>
              <th style={{ textAlign: "left", padding: "14px 16px", color: "var(--color-primary)", fontSize: 13 }}>Driver ID</th>
              <th style={{ textAlign: "left", padding: "14px 16px", color: "var(--color-primary)", fontSize: 13 }}>Driver Name</th>
              <th style={{ textAlign: "left", padding: "14px 16px", color: "var(--color-primary)", fontSize: 13 }}>Date</th>
              <th style={{ textAlign: "left", padding: "14px 16px", color: "var(--color-primary)", fontSize: 13 }}>Car type</th>
              <th style={{ textAlign: "left", padding: "14px 16px", color: "var(--color-primary)", fontSize: 13 }}>Window</th>
              <th style={{ textAlign: "left", padding: "14px 16px", color: "var(--color-primary)", fontSize: 13 }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {records.map((record) => (
              ...
              >
                <td style={{ padding: "14px 16px", color: "var(--color-primary)", fontWeight: 600 }}>{record.driver?.userNumber ? `#${record.driver.userNumber}` : "—"}</td>
                <td style={{ padding: "14px 16px", color: "var(--color-muted)" }}>{record.driver?.name ?? "—"}</td>
                <td style={{ padding: "14px 16px", color: "var(--color-muted)" }}>{record.date ?? "—"}</td>
                <td style={{ padding: "14px 16px", color: "var(--color-muted)" }}>{CAR_TYPE_LABELS[record.driver?.carType ?? ""] ?? "—"}</td>
                <td style={{ padding: "14px 16px", color: "var(--color-muted)" }}>{record.startTime ?? "—"} → {record.endTime ?? "—"}</td>
                <td style={{ padding: "14px 16px" }} onClick={(event) => event.stopPropagation()}>
                  <button
                    type="button"
                    onClick={() => handleDelete(record._id)}
                    disabled={deletingId === record._id}
                    style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 12px", borderRadius: 999, border: "1px solid var(--color-danger)", background: "var(--color-transparent)", color: "var(--color-danger)", cursor: deletingId === record._id ? "not-allowed" : "pointer", fontWeight: 700 }}
                  >
      ...
      {detailId ? (
        ...
        >
          <aside
            ...
            style={{ width: "min(560px, 100%)", height: "100dvh", overflowY: "auto", background: "var(--color-panel)", borderTop: "3px solid var(--color-secondary)", boxShadow: "-12px 0 40px var(--color-shadow-strong)" }}
          >
            <header style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, padding: "20px 24px", borderBottom: "1px solid var(--color-border)" }}>
              <div>
                <p style={{ margin: 0, fontSize: 11, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--color-secondary-deep)" }}>Availability details</p>
                <h3 style={{ margin: "5px 0 0", fontSize: 20, fontWeight: 700, color: "var(--color-primary)" }}>
                  {detail ? `Availability #${detail.availabilityNumber ?? detail._id.slice(-6)}` : "Loading..."}
                </h3>
                {detail ? <p style={{ margin: "4px 0 0", color: "var(--color-muted)", fontSize: 13 }}>{detail.date} · {detail.startTime} → {detail.endTime}</p> : null}
              </div>
              <button type="button" onClick={closeDetail} aria-label="Close details" style={{ padding: 4, color: "var(--color-muted)", background: "var(--color-transparent)", border: "none", cursor: "pointer" }}><X size={20} /></button>
            </header>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: 20 }}>
              {detailLoading ? (
                ...
                <AdminErrorState title="Unable to load availability" description={detailError} />
              ) : detail ? (
                <>
                  {mapPoints.length ? (
                    ...
                    <p style={{ margin: 0, color: "var(--color-muted)", fontSize: 13 }}>No coordinates recorded for this availability.</p>
                  )}

                  <section style={{ border: "1px solid var(--color-border)", borderRadius: 8, padding: 14 }}>
                    <h4 style={{ margin: "0 0 12px", color: "var(--color-primary)", fontSize: 15 }}>Driver</h4>
                    <div className="avail-detail-grid">
                      <div><span className="avail-detail-label">Name</span><span style={{ color: "var(--color-primary)", fontSize: 14 }}>{detail.driverId?.name ?? "—"}</span></div>
                      <div><span className="avail-detail-label">Phone</span><span style={{ color: "var(--color-primary)", fontSize: 14 }}>{detail.driverId?.phone ?? "—"}</span></div>
                      <div><span className="avail-detail-label">Status</span><AdminStatusBadge status={detail.status ?? "unknown"} /></div>
                      <div><span className="avail-detail-label">Matched</span><span style={{ color: "var(--color-primary)", fontSize: 14 }}>{detail.matched ? "Yes" : "No"}</span></div>
                    </div>
                  </section>

                  <section style={{ border: "1px solid var(--color-border)", borderRadius: 8, padding: 14 }}>
                    <h4 style={{ margin: "0 0 12px", color: "var(--color-primary)", fontSize: 15 }}>Locations</h4>
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      <div><span className="avail-detail-label">Start location</span><span style={{ color: "var(--color-primary)", fontSize: 13 }}><MapPin size={12} style={{ verticalAlign: "-1px", color: "var(--color-secondary)" }} /> {detail.startLocation?.address ?? "—"}</span></div>
                      <div><span className="avail-detail-label">Nearest station to start</span><span style={{ color: "var(--color-primary)", fontSize: 13 }}>{detail.startNearestStation?.name ?? "—"}</span></div>
                      <div><span className="avail-detail-label">End location</span><span style={{ color: "var(--color-primary)", fontSize: 13 }}><MapPin size={12} style={{ verticalAlign: "-1px", color: "var(--color-primary)" }} /> {detail.endLocation?.address ?? "—"}</span></div>
                      <div><span className="avail-detail-label">Nearest station to end</span><span style={{ color: "var(--color-primary)", fontSize: 13 }}>{detail.endNearestStation?.name ?? "—"}</span></div>
                    </div>
                  </section>
                </>
              ) : null}
      ...
      {createOpen ? (
        ...
        >
          <AdminFormLayout
            ...
            style={{ display: "flex", flexDirection: "column", gap: 14, width: "min(540px, 100%)", maxHeight: "min(88dvh, 760px)", overflowY: "auto", padding: 20, borderRadius: 14, borderTop: "3px solid var(--color-accent)", background: "var(--color-panel)", boxShadow: "0 20px 60px var(--color-shadow-strong)" }}
          >
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "var(--color-primary)" }}>Add new availability</h3>
                <p style={{ margin: "4px 0 0", color: "var(--color-muted)", fontSize: 13 }}>Assign a driver a new availability window.</p>
              </div>
              <button type="button" onClick={closeCreate} aria-label="Close" disabled={creating} style={{ padding: 4, color: "var(--color-muted)", background: "var(--color-transparent)", border: "none", cursor: "pointer" }}><X size={20} /></button>
            </div>
            ...
            <div className="avail-field">
              ...
              <AddressInput id="avail-end" placeholder="Search end address" value={endLocation} onChange={setEndLocation} iconColor="var(--color-primary)" />
            </div>

            {createError ? <p role="alert" style={{ margin: 0, color: "var(--color-danger)", fontSize: 13 }}>{createError}</p> : null}
