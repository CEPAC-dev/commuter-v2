...
const VERIFICATION_META: Record<VerificationStatus, { label: string; tone: ToneKey; Icon: LucideIcon }> = {
...
const SAMPLE_USERS: UserRow[] = [
  {
    ...
    driver: {
      ...
      carCapacity: 4,
      documents: {
        nationalIdFront: "https://images.unsplash.com/photo-1586771107445-d3ca888129ff?w=400",
        drivingLicense: "https://images.unsplash.com/photo-1586771107445-d3ca888129ff?w=400",
        carImage: "https://images.unsplash.com/photo-1541443131876-44b03de101c5?w=400",
      },
    },
  },
  {
...
export default function UserManagementClient({
  ...
  const summary = useMemo(() => {
    ...
    const pendingCount = rows.filter((r) => r.driver?.verificationStatus === "pending").length;
  ...
  function updateRow(userId: string, patch: Partial<UserRow>) {
    setRows((current) => current.map((row) => (row._id === userId ? { ...row, ...patch } : row)));
  }

  async function saveChanges(user: UserRow) {
    const userId = user._id;
    setSavingIds((current) => ({ ...current, [userId]: true }));
    setFeedback((current) => ({ ...current, [userId]: { type: "success", message: "Saving…" } }));
    ...
    try {
      ...
      if (user.driver) {
        payload.verificationStatus = user.driver.verificationStatus ?? "incomplete";
      }
      ...
      setFeedback((current) => ({
        ...current,
        [userId]: { type: "error", message: error instanceof Error ? error.message : "Unexpected error." },
      }));
    } finally {
      ...
      <div className="flex flex-col gap-5 border-b border-slate-100 p-6 sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <StatPill icon={Users} tone="navy" label="Total" value={summary.total} />
            <StatPill icon={Car} tone="teal" label="Drivers" value={summary.driverCount} />
            <StatPill icon={ShieldCheck} tone="slate" label="Admins" value={summary.adminCount} />
            {summary.pendingCount > 0 && (
              <StatPill icon={Clock} tone="amber" label="Pending review" value={summary.pendingCount} />
            )}
          </div>
        </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <InfoCard title="Admin controls">
                        <FieldSelect
                          ...
                          onChange={(value) => updateRow(user._id, { role: value })}
                        ...
                        {user.driver && (
                          <FieldSelect
                            ...
                            value={user.driver.verificationStatus || "incomplete"}
                            onChange={(value) =>
                              updateRow(user._id, {
                                driver: {
                                  ...(user.driver ?? {}),
                            ...
                            optionLabels={Object.fromEntries(
                              Object.entries(VERIFICATION_META).map(([k, v]) => [k, v.label])
                            )}
                          />
                        )}
                      ...
                      <>
                        <InfoCard title="Driver details">
                          <div className="grid grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-4">
                            <MiniStat icon={Car} label="Car" value={`${user.driver.carBrand || "—"} ${user.driver.carModel || ""}`.trim()} />
                            <MiniStat icon={Gauge} label="Capacity" value={user.driver.carCapacity ?? "—"} />
                            <MiniStat icon={Palette} label="Color" value={user.driver.vehicleColor || "—"} />
                            <MiniStat icon={CalendarDays} label="License exp." value={user.driver.licenseExpiry || "—"} />
                          </div>
                          <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
                            <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Plate</span>
                            ...
                            <span className="ml-2 text-xs text-slate-500">
                              {user.driver.carType || "—"} · {user.driver.modelYear || "—"}
                            </span>
                          </div>
                        </InfoCard>

                        <InfoCard title="Uploaded documents">
                          {user.driver.documents && Object.values(user.driver.documents).some(Boolean) ? (
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                              {Object.entries(user.driver.documents ?? {}).map(([key, value]) => {
                                if (!value) return null;
                                const docLabel = key in DOC_LABELS ? DOC_LABELS[key] : key;
...
function buildPlate(driver?: DriverProfile) {
  const parts = [driver?.plateChar1, driver?.plateChar2, driver?.plateChar3, driver?.plateDigits].filter(Boolean);
  return parts.join(" ");
}
...
function StatPill({ icon: Icon, label, value, tone }: { icon: LucideIcon; label: string; value: number | string; tone: ToneKey }) {
  return (
    <div className={`flex items-center gap-2 rounded-full px-3.5 py-2 ring-1 ring-inset ${TONE_CLASSES[tone]}`}>
...
function RoleBadge({ role }: { role?: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold capitalize ring-1 ring-inset ${TONE_CLASSES[tone]}`}>
      {role || "passenger"}
    </span>
  );
}
...
function InfoRow({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value?: string | number | null }) {
  return (
...
function MiniStat({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value?: string | number | null }) {
  return (
...
function FieldSelect({ label, value, onChange, options, optionLabels }: { label: string; value: string; onChange: (value: string) => void; options: string[]; optionLabels?: Record<string, string> }) {
  return (
